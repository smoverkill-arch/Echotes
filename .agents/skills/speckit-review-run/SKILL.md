...
4. **Identify Changed Files**
   ...
   - Otherwise, you **MUST** execute the `.specify/scripts/powershell/detect-changed-files.ps1` with `--json` to detect changed files. **Do not** attempt to detect changes by running `git` commands directly, reading git state manually, or using any other method — always delegate to the script.
